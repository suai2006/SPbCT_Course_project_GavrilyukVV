...
module.exports = class Logger
{
    ...
    middleware(prop) 
    {
        this[prop] = (msg) => 
        {
            ...
            let filePath = `${this.logs_path}/${prop}`;
            this.mkDir(filePath);
            let file = `${filePath}/${prop}-${this.date}.log`;
            let level = "info";            
    ...
    getFunc(msg)
    {
        let stackFilter = (f) => 
        {
            let check = f.indexOf(__filename) == -1 && f.indexOf('Error') == -1 && f.indexOf('node:') == -1;
            return check
        }

        let error = msg;
        if(Object.prototype.toString.call(msg) !== '[object Error]') error = new Error(msg);
    ...
    mkDir(logs_path)
    {
        ...
        { 
            ...
            {
                let pathList = logs_path.split(path.sep).filter(Boolean);
